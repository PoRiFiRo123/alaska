...
client.on("ready", async () => {
    const ownerId = await client.users.fetch("758564566035988511").catch(() => null);
    if (!ownerId) {
    ...
    const guild = await client.guilds.fetch("929959443607748689").catch(() => null);
    if (!guild) {
        throw new Error("The ServerID value you provided in mainconfig.js is invalid!");
    }

    const memberRoleId = await guild.roles.fetch("941226271961124896").catch(() => null);
    if (!memberRoleId) {
