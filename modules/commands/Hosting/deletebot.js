...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (!message.member.roles.cache.has(`${"990525076413743184"}`)) return message.reply({
            content: `<a:no:989961174546268190> **You Are Not Allowed To Execute This Command, Only Owners!**`
        })
        try {
            const logs = client.channels.cache.get(`${"990592712879276052"}`)
            ...
            if (!bot || !bot.id) return message.channel.send("**<a:no:989961174546268190> To Delete The Folder, Please Mention a Bot!**");
            ...
            collector.on("collect", (b) => {
                if (b.user.id !== message.author.id)
                    return b.reply({
                        content: `<a:no:989961174546268190> **Only <@${message.author.id}> Allowed To React To This Message!**`,
                        ephemeral: true
                    })
                if (b.isButton()) {
