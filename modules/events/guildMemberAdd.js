...
module.exports = async (client) => {
    ...
    client.on("guildMemberAdd", async member => {
        ...
        if (member.guild && member.user.bot) {
            ...
            if (member.permissions.has(FLAGS.ADMINISTRATOR) || member.permissions.has(FLAGS.MANAGE_GUILD) || member.permissions.has(FLAGS.KICK_MEMBERS) || member.permissions.has(FLAGS.BAN_MEMBERS) || member.permissions.has(FLAGS.MANAGE_CHANNELS) || member.permissions.has(FLAGS.MANAGE_ROLES) || member.permissions.has(FLAGS.MANAGE_WEBHOOKS)) {
                ...
                const WhitelistedUsers = [`${"758564566035988511"}`];
    ...
    client.on("guildMemberAdd", member => {
        ...
        if (member.user.bot) {
            setTimeout(async () => {
                member.roles.set([`${"941226271961124896"}`]).catch(() => { })
            }, 5000)
            return;
        }
        if (!member.user.bot) {
            try {
                welcomeChannel.send(`👋! ${member.user} welcome to **Astrom**`);
                member.send({
                    embeds: [new Discord.MessageEmbed()
                        ...
                        .setDescription(`
Welcome ${member.user} to <:Discord:989923995447099392>  [Astrom Development](https://bit.ly/3NJDSFP)<:Discord:989923995447099392>!
