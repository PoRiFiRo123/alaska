...
module.exports = client => {
  ...
  var job3 = new CronJob('0 */15 * * * *', function () {
    client.channels.fetch(`${mainconfig.OrdersChannelID.TicketChannelID}`).then(ch => {
      ch.messages.fetch(`${mainconfig.OrdersChannelID.TicketMessageID}`).then(async msg => {
        ...
        if (msg.embeds[0].fields && msg.embeds[0].fields.length > 0) {
          msg.embeds[0].fields[0].name = `<a:Online_Ping:993084931515359263> **[${onlinesupporters.length}] Supporters online:**`;
          ...
          msg.embeds[0].addField(`<a:Online_Ping:993084931515359263> **[${onlinesupporters.length}] Online Supporter:**`, onlinesupporters.length > 25 ? +onlinesupporters.slice(0, 25).join(", ") + `, ${onlinesupporters.length - 25} ***More...***` : onlinesupporters.join(", "))
        }
        msg.edit({
          embeds: [msg.embeds[0]]
        }).catch((e) => {
    ...
    setTimeout(async () => {
      ...
      //ORDER MESSAGE
      client.channels.fetch("993064775187693608").then(ch => {
