...
module.exports = client => {
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if(interaction.customId == "closeticket"){
            interaction.reply({content: `$ **Now Closing the Ticket!**\n> Don't forget to leave <#${mainconfig.FeedBackChannelID.toString()}> and a review on our <https://de.trustpilot.com/review/nexusx.me>`, ephemeral: true}).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
            setTimeout(async() => {
                if(channel) {
                    let userid = false;
                    try {
